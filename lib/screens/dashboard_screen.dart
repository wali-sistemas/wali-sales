...
class _DashboardPageState extends State<DashboardPage> {
 ...
 Future<Map<String, dynamic>> _datosDashboard2() async {
   ...
   final String apiUrl = 'http://wali.igbcolombia.com:8080/manager/res/app/budget-sales/'+empresa!+'?slpcode='+usuario!+'+&year='+now.year.toString()+'&month='+now.month.toString();
   ...
   data={"code":0,"content":[{"year":2023,"slpCode":"56","companyName":"IGB","month":"04","ventas":00.00,"presupuesto":00,"pendiente":00.00,"whsDefTire":"01"}]};
   return data;
   }
 ...
 Future<Map<String, dynamic>> _datosBarras() async {
   ...
   final String apiUrl = 'http://wali.igbcolombia.com:8080/manager/res/app/effectiveness-sales/'+empresa!+'?slpcode='+usuario!+'+&year='+now.year.toString()+'&month='+now.month.toString();
   ...
   Map<String, dynamic> data ={};
   if (!resp1["content"].toString().contains("Ocurrio un error") || !resp1["content"].toString().contains("No se encontraron")) {
     return resp1;
     ...
     data={"code":0,"content":[{"slpCode":"56","slpName":"SEBASTIAN KIZA LANCHEROS","year":2023,"month":1,"base":0,"impact":0,"effectiveness":0.0}]};
     return data;
   }
            ...
            FutureBuilder<Map<String, dynamic>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  ...
                  storage.write("nombreAsesor", snapshot.data!["content"][0]["slpName"] );
                  storage.write("emailAsesor", snapshot.data!["content"][0]["mail"]);
                  storage.write("zona", snapshot.data!["content"][0]["whsDefTire"]);

                  storage.write("urlFoto", snapshot.data!["content"][0]["urlSlpPicture"]);
                  if(presupuestoT==0 || presupuestoT.isNaN || presupuestoT==null)
                    {dataMap["Ventas"] = 0;
                    dataMap["Presupuesto"] =0;
                    dataMap["Pend. por facturar"] = 0;
                    } else {
                    ventasPendientes=(ventasT/presupuestoT)*100;
                    ventasPendientes=double.parse((ventasPendientes).toStringAsFixed(2));
                    dataMap["Ventas"] = ventasT;
                    dataMap["Presupuesto"] = presupuestoT.toDouble() - ventasT;
                  ...
                  return Column(
                    children: [ SizedBox( height: 50 ),
                      Text("Presupuesto",style: TextStyle(
                        fontSize: 20,
                      ),),
                      SizedBox( height: 20 ),
                      Text(presupuestoTstr,style: TextStyle(
                        fontSize: 20,
                      ),),
                      Text("Presupuesto total"),
                      SizedBox( height: 50 ),
                      Center(child: Text('Ventas $ventasPendientes %',
                ...
                child:
                      Row( crossAxisAlignment: CrossAxisAlignment.center,
                        ...
                        children: <Widget>[
                          ...
                          Expanded(
                            child: Text(ventastStr+'\n    Ventas',style: TextStyle(
                                fontSize: 18)
                            ),
                          ),
                          Expanded(
                            child:  Text(presupuestoPstr+'\nPend. por facturar',style: TextStyle(
                              fontSize: 18,
                            ),),
                          ),
            ...
            FutureBuilder<Map<String, dynamic>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                   ...
                   print("datos ---------------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>--------------------");print(snapshot.data!["content"].toString());
                  // print (snapshot.data!["content"][0]["ventas"]);
                  // print (snapshot.data!["content"][0]["slpName"]);
                  if (snapshot.data!["content"].toString().contains("Ocurrio un error") || snapshot.data!["content"]=="No se encontraron datos para graficar la efectividad.") {
                    ...
                    children: [
                          ...
                          children: [
                            ...
                            SizedBox(height: 45),
                             Row(
                              ...
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
                                BarraGrafica(color: Colors.blue, valor: base.toDouble(), etiqueta: base.toString()),
                                SizedBox(width: 16),
                                BarraGrafica(color: Colors.yellow, valor: impacto.toDouble(), etiqueta: impacto.toString()),
                              ],
                            ),
                          ],
                        ),
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 16),
                          EtiquetaBullet(color: Colors.yellow, texto: 'Clientes efectivos'),
                          SizedBox(height: 16),
                          EtiquetaBullet(color: Colors.blue, texto: 'Presupuesto \nde clientes'),
...
class BarraGrafica extends StatelessWidget {
  ...
  const BarraGrafica({required this.color, required this.valor, required this.etiqueta});
