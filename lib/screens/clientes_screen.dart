...
class _ClientesPageState extends State<ClientesPage> {
  ...
  Future<void> _fetchData() async {
    if (GetStorage().read('datosClientes') == null) {
      ...
      final response = await http.get(Uri.parse(apiUrl));
      Map<String, dynamic> resp = jsonDecode(response.body);
      String texto = "No se encontraron clientes para usuario " + codigo +" y empresa " + empresa;
    ...
    } else {_clientes=GetStorage().read('datosClientes');}
  }
          ...
          itemBuilder: (context, index) {
            return Card(
              child: Padding(
                ...
                child: ListTile(
                  title: Text(_clientes[index]['cardCode']+' - '+
                  ...
                  trailing: TextButton.icon(
                    onPressed: () {
                      ...
                      print (pedidoLocal["cardCode"]);
                      print(_clientes[index]['cardCode']);
                      print(itemsPedidoLocal.length);
                      print (_clientes[index]["nit"]);
                      if (pedidoLocal["cardCode"]!=_clientes[index]['cardCode'] && itemsPedidoLocal.length>0) {
                        //showAlertDialog(context, _clientes[index]["nit"]);
  ...
  showAlertDialog(BuildContext context,String nit) {
    ...
    Widget cancelButton = ElevatedButton(
      child: Text("NO"),
      onPressed:  () { storage.write('nit',nit);Navigator.pop(context);},
    );
    ...
    AlertDialog alert = AlertDialog(
      title: Text("Atención"),
      content: Text("Tiene ítmes pendientes para otro cliente, si continúa se borrarán e inciará un pedido nuevo, desea continuar?"),
      actions: [
        cancelButton,
        continueButton,
      ],
